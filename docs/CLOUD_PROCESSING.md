...
| WV courthouse researcher worker | Hourly | WV runbook/toolkit release hardening that does not touch live shared-drive data |
...
4. `wv-courthouse-researcher`: private WV courthouse research repo, `codex/wv-courthouse-researcher-cabell-lessons`.
...
- Public control-repo workflow `CertaHealth Control Checks` is visible on GitHub and the latest visible run observed on June 21, 2026 was successful.
- The latest visible overall run is `#127` on branch `codex/release-control-webapp-path-guard-20260621-1500`, commit `9cd16ff`, success.
- The latest previously verified `main` run is `#40` on branch `main`, commit `e0cd20b`, success.
