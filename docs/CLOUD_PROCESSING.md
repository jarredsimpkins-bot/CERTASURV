...
- Public control-repo workflow `CertaHealth Control Checks` is visible on GitHub and the latest public runs observed on June 23, 2026 were successful.
- The latest visible `main` run is `#41` on branch `main`, commit `6f40b64`, success.
- The latest visible overall public run during the June 23 pass is `#200` on branch `codex/release-control-webapp-path-guard-20260623-selfsafe`, commit `21ffa9f`, success.
- GitHub CLI is authenticated as `jarredsimpkins-bot`, but the visible token scopes do not include explicit `workflow`; confirm scope before relying on private workflow/log inspection for release gates.
- Local provisioning is not fully ready for launch handoff: the shared-drive `G:` mount and `npm` on PATH are missing in this shell.
