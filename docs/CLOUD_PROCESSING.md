...
- Public control-repo workflow `CertaHealth Control Checks` is visible on GitHub and the latest overall run observed on June 22, 2026 was successful.
- The latest visible overall run is `#157` on branch `codex/release-control-webapp-path-guard-20260622-automation`, commit `ad49013`, success.
- The latest visible `main` run is `#41`, commit `6f40b64`, success.
- Private-repo workflow outputs are available in this automation context through authenticated `gh`: CERTARD is green, MACROTBC is red on folder-map/shared-config readiness, and CERTASURV_WEB_APP needs a fresh rerun after its June 19 path-handling failure.
