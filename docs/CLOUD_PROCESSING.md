...
5. `certasurv-web-app`: private app/dashboard repo, `codex/land-opportunity-radar-mvp`, local path `C:\Users\SimpS\OneDrive\Documents\CERTASURV_WEB_APP` with legacy `New project2` fallback in helper scripts.
...
- Public GitHub Actions page for `jarredsimpkins-bot/CERTASURV` showed 24 `CertaHealth Control Checks` runs on 2026-06-17, including run `#24` for commit `a89a5cb` on branch `codex/certasurv-web-app-rename-alignment`.
- Local `gh auth status` remains unauthenticated on 2026-06-17, so private-repo workflow logs and artifact inspection still require either `gh auth login` or another authenticated path.
- Shared-drive evidence is present locally: `G:\Shared drives\CERTASURV_PROJECT DRIVE` is mounted, `G:\Shared drives\CERTASTRUCT` is mounted, and the latest `TBC_WEB_INSTALLERS` package seen during this pass was `CertaSurv_TBC_Web_Installer_20260522-221032.zip`.
