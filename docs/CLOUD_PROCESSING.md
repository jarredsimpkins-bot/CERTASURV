...
- Public control-repo workflow `CertaHealth Control Checks` is visible on GitHub and the latest `main` run inspected on June 23, 2026 was successful.
- The latest verified `main` run is `#41`, commit `6f40b64`, success.
- The latest verified overall run is `#181` on branch `codex/release-ops-blocker-refresh-20260623-0058`, commit `8da6dba`, success.
- Private-repo workflow outputs are available while the current authenticated `gh` session remains valid.
