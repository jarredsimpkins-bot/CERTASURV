...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Local-only courthouse research lane; remote target blocked until explicitly configured | Courthouse, deed, parcel, and IDX/browser-capture source/docs | Locally coherent, remote-blocked |
...
| WV_COURTHOUSE_RESEARCHER has no configured remote | Courthouse source/docs can be improved locally but cannot be pushed by launch automation | Add an intentional remote when the lane is ready for hosted source control |
