...
| Outside courthouse research | WV courthouse research toolkit, source registry, and basemap prep | `WV_COURTHOUSE_RESEARCHER` docs, scripts, registry, and eventual GitHub remote |
...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Planned `origin` -> `https://github.com/jarredsimpkins-bot/wv-courthouse-researcher.git`; currently no remote configured | Courthouse toolkit, project prep docs, TBC basemap workflow | Active blocker: local modified files need upstream/review path |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared drive mount is currently unavailable | Outside system-of-record folders cannot be verified or staged from this session | Restore Google Drive Desktop mount for `G:\Shared drives\CERTASURV_PROJECT DRIVE`, then rerun provisioning |
...
| WV_COURTHOUSE_RESEARCHER has no configured remote | Release branch cannot be pushed or reviewed from the local repo yet | Create/confirm the GitHub repo, run `Set-CertaGitRemotes.ps1 -Apply`, then push the active branch |
| MACROTBC has a modified command usage log | Release blocker triage needs to decide whether it is intentional evidence or local runtime noise | Commit only if it is release evidence; otherwise exclude it from release commits |
| GitHub CLI auth is present but should be rechecked before private operations | `gh`-driven release pushes and private workflow/log inspection depend on the current token/session | Run `gh auth status` before private workflow/log inspection or release pushes |
