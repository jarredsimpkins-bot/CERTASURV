...
| CERTARD | `C:\Users\SimpS\OneDrive\Documents\CERTARD` | Shared-drive mount helpers; `origin` -> `https://github.com/jarredsimpkins-bot/certard.git` | Project watchlist, staging scripts | Active coordination repo; clean branch awaiting push/review timing |
...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No `origin` configured yet; upstream target still undecided | Parcel/courthouse research automation, prep machine docs, toolkit registry | Active research repo; current launch blocker until upstream is established |
...
| `MACROTBC` release branch is not clean | TBC integration release work cannot be pushed/reviewed safely until tracked and untracked changes are triaged | Commit or explicitly discard non-release changes in `MACROTBC` before launch |
| `WV_COURTHOUSE_RESEARCHER` is local-only | Release ops cannot offload or review that repo until an upstream remote exists | Create the GitHub repo or set `origin`, then push the active branch |
