...
| CERTARD | `C:\Users\SimpS\OneDrive\Documents\CERTARD` | Shared-drive mount helpers; `origin` -> `https://github.com/jarredsimpkins-bot/certard.git` | Project watchlist, staging scripts | Active coordination repo; unrelated local edit present outside this repo |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared drive mount unavailable in this session | Final handoff validation and generated documentation copies cannot be verified against `G:` here | Restore Google Drive Desktop mount for `G:\Shared drives\CERTASURV_PROJECT DRIVE` before launch handoff |
...
| `pwsh`, Node, and npm are missing from PATH in this session | Checklist parity and Node web tooling may fail outside the expected release shell | Normalize host PATH or install/reinstall PowerShell 7 and Node.js LTS when the machine is not under production load |
