...
| Deed/courthouse research | WV deed, tax map, KML, and courthouse packet workflows | `WV_COURTHOUSE_RESEARCHER` plus project packet docs |
...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No `origin` remote observed on June 20, 2026; planned slug `wv-courthouse-researcher` | Deed packet, KML, tax map, and courthouse/TBC basemap scripts | Active release blocker repo |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared drive mount unavailable in this run | Local provisioning cannot see `G:\Shared drives\CERTASURV_PROJECT DRIVE` or command-center project folders | Restore Google Drive Desktop mount before shared-drive handoff or staging validation |
...
| MACROTBC shared-drive config missing locally | `C:\Users\SimpS\OneDrive\Documents\MACROTBC\certasurv_shared_drive.json` is absent during provisioning | Classify MACROTBC deleted tracked roots before launch push |
| WV_COURTHOUSE_RESEARCHER has no observed remote/upstream | WV release work cannot be pushed or reviewed from the normal launch lane | Create/confirm `jarredsimpkins-bot/wv-courthouse-researcher`, then run `scripts\Set-CertaGitRemotes.ps1 -Apply` |
| Node/npm unavailable in this shell | Web tooling cannot run from this host shell without PATH or install repair | Normalize host PATH or install/reinstall Node.js LTS when the machine is not under production load |
