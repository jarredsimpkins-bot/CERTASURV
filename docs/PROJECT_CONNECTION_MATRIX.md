...
| CERTARD | `C:\Users\SimpS\OneDrive\Documents\CERTARD` | Shared-drive mount helpers; `origin` -> `https://github.com/jarredsimpkins-bot/certard.git` | Project watchlist, staging scripts | Active coordination repo; local tree has unrelated dirty liaison brief work |
| MACROTBC | `C:\Users\SimpS\OneDrive\Documents\MACROTBC` | AppSheet, Google Drive, command-center manifest; `origin` -> `https://github.com/jarredsimpkins-bot/macrotbc.git` | TBC macros, CAD resources, installers, sync scripts | Active production integration repo; latest readiness workflow failing |
...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No remote configured; expected repo name is not resolvable through `gh` | Courthouse/title research and CertaSurv toolkit docs/scripts | Local-only blocker with unrelated dirty toolkit docs/scripts |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared drive not mounted in current host session | Outside system-of-record folders cannot be verified or handed off from this session | Sign into/remount Google Drive for Desktop before shared-drive package handoff; historical stage log path remains `G:\Shared drives\CERTASURV_PROJECT DRIVE\00_CERTASURV_COMMAND_CENTER\08_REPORTS_EXPORTS\drive-stage-log-20260522-194113.txt` |
...
| MACROTBC readiness workflow failing | TBC release package cannot be marked launch-ready | Fix command registry, TBC operator workflow HTML, and TBC tabulated workflow checks in `macrotbc` |
| WV_COURTHOUSE_RESEARCHER has no remote | Courthouse researcher cannot be pushed/reviewed as part of launch | Choose/create the GitHub destination and set `origin` |
| GitHub CLI is authenticated on this host | Private workflow/log inspection is available now | Keep token/session active for release operations |
