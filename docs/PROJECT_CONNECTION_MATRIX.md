...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No accessible `origin` confirmed during the June 23 release-ops pass | Courthouse/title research tooling and assessor prep docs | Substantive release blocker until upstream and review path are fixed |
...
| GitHub CLI is authenticated on this host | `gh`-driven private workflow/log inspection is available for repos the account can access | Re-check `gh auth status` before push/release operations |
| MACROTBC workflow readiness is failing | TBC macro/package release cannot be treated as launch-ready while `CertaSurv TBC Cloud Readiness` is red | Fix failing MACROTBC runs before package handoff |
