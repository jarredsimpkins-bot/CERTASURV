...
| Outside courthouse research | County records package and source docs | `WV_COURTHOUSE_RESEARCHER` local repo plus planned GitHub remote |
...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Planned `origin` -> `https://github.com/jarredsimpkins-bot/wv-courthouse-researcher.git`; local checkout currently has no remote | Courthouse research scripts/docs | Local source coherent; remote setup blocked |
...
| WV_COURTHOUSE_RESEARCHER has no local origin | Control scripts can inventory the lane, but cloud/offload publishing cannot push it from this host | Add the planned remote when the repository is ready to publish |
