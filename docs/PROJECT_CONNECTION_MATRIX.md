...
| In-house courthouse research | Deed, title, legal description, and county-record research tools | `WV_COURTHOUSE_RESEARCHER` source package |
...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Planned `origin` -> `https://github.com/jarredsimpkins-bot/wv-courthouse-researcher.git`; current checkout is local-only | Courthouse research scripts/docs and packaging | Source coherent; remote-blocked locally |
...
| WV_COURTHOUSE_RESEARCHER has no local `origin` | Control scripts can inventory it, but cannot push it from this host until a remote is attached | Configure `origin` after the private GitHub target is ready |
