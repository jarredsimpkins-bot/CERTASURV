...
| CERTARD | `C:\Users\SimpS\OneDrive\Documents\CERTARD` | Shared-drive mount helpers; `origin` -> `https://github.com/jarredsimpkins-bot/certard.git` | Project watchlist, staging scripts | Active coordination repo; push/review timing is the main release concern |
| MACROTBC | `C:\Users\SimpS\OneDrive\Documents\MACROTBC` | AppSheet, Google Drive, command-center manifest; `origin` -> `https://github.com/jarredsimpkins-bot/macrotbc.git` | TBC macros, CAD resources, installers, sync scripts | Blocked by missing shared-drive config and local churn |
...
| CERTASURV_WEB_APP | `C:\Users\SimpS\OneDrive\Documents\CERTASURV_WEB_APP` | Shared-drive project data and estimate folders; `origin` -> `https://github.com/jarredsimpkins-bot/certasurv-web-app.git` | Local Python app/dashboard tools | Active local app project after workspace rename; push/review timing is the main release concern |
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No remote configured in inspected workspace | Deed/title/legal and TBC basemap preparation tooling | Blocked until remote/upstream is configured |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared drive mount is unavailable in this shell | Outside system-of-record folders cannot be validated from this host session | Restore `G:\Shared drives\CERTASURV_PROJECT DRIVE` before shared-drive handoff validation |
...
| MACROTBC shared-drive config is deleted locally | TBC release/install handoff cannot be considered clean until the config path is restored or intentionally replaced | Reconcile `certasurv_shared_drive.json` in the MACROTBC repo |
| WV_COURTHOUSE_RESEARCHER has no remote in the inspected workspace | Toolkit/runbook fixes cannot be pushed for review from that clone | Add the intended GitHub remote and upstream branch |
