...
| --- | --- | --- | --- | --- |
| CERTAHEALTH | `C:\Users\SimpS\OneDrive\Documents\CERTAHEALTH` | `origin` -> `https://github.com/jarredsimpkins-bot/CERTASURV.git`; planned dedicated target `certahealth.git` currently returns `repository not found` | Laptop load manager, project provisioning scripts | Active local control repo; remote-target decision still open and dedicated target is not yet provisioned |
...
| CERTAHEALTH remote target is unresolved and dedicated target is absent | Release notes and automation still need a final answer on whether the control repo stays on public `CERTASURV.git` or moves to `certahealth.git`; the dedicated repository URL is not live today | Decide the permanent GitHub destination and create `certahealth.git` before final launch cutover if that path is chosen |
...
| `MACROTBC` and `WV_COURTHOUSE_RESEARCHER` remain dirty implementation lanes | Stack launch readiness is now constrained more by those repos than by `CERTARD` or `CERTASURV_WEB_APP` | Finish, review, or explicitly defer those repo changes before launch cutover |
