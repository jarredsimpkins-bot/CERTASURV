...
| Outside operations drive | System-of-record folders, IDs, releases, and handoffs | `G:\Shared drives\CERTASTRUCT` with `G:\Shared drives\CERTASURV_PROJECT DRIVE` as legacy fallback |
...
| In-house local apps | Local web/dashboard/parcel/estimate tools | `CERTASURV_WEB_APP` plus local Python runtime; `New project2` remains a fallback alias |
...
| CERTASURV_WEB_APP | `C:\Users\SimpS\OneDrive\Documents\CERTASURV_WEB_APP` with fallback `C:\Users\SimpS\OneDrive\Documents\New project2` | Shared-drive project data and estimate folders; Git remote should stay `certasurv-web-app` | Local Python app/dashboard tools | Active local app project |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared-drive root rename is mid-cutover | Release docs and scripts must tolerate both roots until the legacy mount is retired | Prefer `G:\Shared drives\CERTASTRUCT`; keep `G:\Shared drives\CERTASURV_PROJECT DRIVE` as fallback |
...
| GitHub CLI installed but this workstation is not authenticated | GitHub auth/status/push automation is available after `gh auth login`; `CERTAHEALTH` also still points at the legacy public `CERTASURV` remote in older checkouts | Sign into `gh`, then switch `CERTAHEALTH` to `https://github.com/jarredsimpkins-bot/certahealth.git` |
