...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Remote/upstream not configured in the current checkout | Courthouse research scripts, toolkit docs, ortho/LiDAR workflow notes | Release blocker until local edits are reviewed and remote is configured |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared drive not mounted locally | Outside system-of-record folders cannot be checked or staged from this host until `G:` is mounted | Mount `G:\Shared drives\CERTASURV_PROJECT DRIVE`, then re-run provisioning and compare against the latest stage log |
...
| WV_COURTHOUSE_RESEARCHER has no remote/upstream here | Launch-toolkit edits cannot be pushed or reviewed from this checkout until repository ownership is decided | Configure the intended GitHub remote, review local edits, then publish through the normal PR/review path |
...
| `pwsh` is not on PATH in this shell | Operator docs that assume PowerShell 7 fail locally even though Windows PowerShell can run the provisioning script | Install PowerShell 7 or use the documented Windows PowerShell fallback command |
