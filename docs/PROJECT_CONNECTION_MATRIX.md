...
| CERTASURV_WEB_APP | `C:\Users\SimpS\OneDrive\Documents\CERTASURV_WEB_APP` | Shared-drive project data and estimate folders; `origin` -> `https://github.com/jarredsimpkins-bot/certasurv-web-app.git` | Local Python app/dashboard tools | Active local app project; mainly push/review timing |
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Release docs updated locally; remote/upstream still needs confirmation before push | Courthouse/deed/title research tooling | Substantive release blocker |
...
| MACROTBC has release-blocking local churn | Tracked root-file deletions and generated outputs need human review before it can be treated as clean release input | Review `git status --short` in `MACROTBC`, restore intentional tracked files or document removals, and ignore/generated-output policy before push |
| WV_COURTHOUSE_RESEARCHER remote/upstream is not confirmed | Prior release-readiness docs cannot be pushed or reviewed until a destination branch exists | Set/confirm `origin` and upstream branch, then push the release-readiness commit for review |
