...
| Outside records research | Courthouse, IDX, deed, and MapWV research workflow | `WV_COURTHOUSE_RESEARCHER` runbooks and toolkit |
...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No local `origin` configured during June 21, 2026 release-ops pass; planned slug should be confirmed before push | Research toolkit, courthouse runbooks, MapWV/IDX workflow notes | Active local blocker repo with release-relevant edits |
...
| MACROTBC local tree is dirty with tracked root deletions and generated artifacts | Release review cannot distinguish intentional source changes from build/runtime output | Triage `git status --short`; commit only intentional source/runbook changes and leave generated output out |
| WV_COURTHOUSE_RESEARCHER has no origin remote configured | Current researcher runbook/toolkit edits cannot be pushed or reviewed from this host | Confirm the private GitHub slug, then run `scripts\Set-CertaGitRemotes.ps1 -Apply` or add the remote manually |
