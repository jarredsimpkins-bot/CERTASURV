...
| CERTARD | `C:\Users\SimpS\OneDrive\Documents\CERTARD` | Shared-drive mount helpers; `origin` -> `https://github.com/jarredsimpkins-bot/certard.git` | Project watchlist, staging scripts | Active coordination repo; mostly push/review timing |
...
| CERTASURV_WEB_APP | `C:\Users\SimpS\OneDrive\Documents\CERTASURV_WEB_APP` | Shared-drive project data and estimate folders; `origin` -> `https://github.com/jarredsimpkins-bot/certasurv-web-app.git` | Local Python app/dashboard tools | Active local app project; mostly push/review timing |
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Remote not visible in local release-ops status output | Courthouse/title research package, OCR/toolbox runbooks | Substantive release blocker pending remote/readiness approval |
...
| MACROTBC release readiness is unresolved | TBC command-center and production integration artifacts can block launch if pushed without review | Review active `command_center/tbc/command_usage_log.csv` change and related command-center/TBC docs before release handoff |
| WV_COURTHOUSE_RESEARCHER release readiness is unresolved | Courthouse package cannot be treated as launch-ready without remote and dirty-tree approval | Confirm final remote, branch, and modified docs/scripts/templates before push/review |
