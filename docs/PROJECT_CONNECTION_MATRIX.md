...
| MACROTBC | `C:\Users\SimpS\OneDrive\Documents\MACROTBC` | AppSheet, Google Drive, command-center manifest; `origin` -> `https://github.com/jarredsimpkins-bot/macrotbc.git`; shared-drive config currently missing in provisioning check | TBC macros, CAD resources, installers, sync scripts | Active production integration repo and release blocker |
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Courthouse research source packages and local repo handoff; upstream tracking not shown in current status | Researcher toolkit, TBC basemap workflow docs, tool registry | Active release blocker until toolkit/runbook edits are closed |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared drive mount missing in current host session | Outside system-of-record folders cannot be validated from this run | Restore `G:\Shared drives\CERTASURV_PROJECT DRIVE` before handoff validation or release copying |
...
| MACROTBC release diff is not closed | Tracked package/config deletions, missing `certasurv_shared_drive.json`, generated outputs, and command-center CSV deltas still need an intentional release decision | Resolve in MACROTBC repo before launch handoff |
| WV_COURTHOUSE_RESEARCHER release diff is not closed | Toolkit/runbook/tool registry edits are active and upstream tracking was not shown in current status | Finish or clear the edits, confirm upstream, and rerun researcher smoke checks |
