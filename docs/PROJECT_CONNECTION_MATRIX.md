...
Last updated: 2026-06-17 00:00 America/New_York
...
| Outside operations drive | System-of-record folders, IDs, releases, and handoffs | `G:\Shared drives\CERTASTRUCT` with fallback to `G:\Shared drives\CERTASURV_PROJECT DRIVE` until migration is complete |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared-drive rename still in transition | Operators can see both `CERTASTRUCT` and `CERTASURV_PROJECT DRIVE`, so runbooks must stay dual-root aware | Latest stage log still resolves under `G:\Shared drives\CERTASURV_PROJECT DRIVE\00_CERTASURV_COMMAND_CENTER\08_REPORTS_EXPORTS\drive-stage-log-20260522-194113.txt` |
