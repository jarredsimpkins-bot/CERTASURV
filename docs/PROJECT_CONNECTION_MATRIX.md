...
| GitHub CLI is authenticated on this host | `gh`-driven release pushes and private workflow/log inspection are available while the current credential remains valid | Confirm `gh auth status` before private workflow inspection or release pushes |
