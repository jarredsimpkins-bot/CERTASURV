...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Planned `origin` -> `https://github.com/jarredsimpkins-bot/wv-courthouse-researcher.git`; currently missing local `origin` | Courthouse runbooks, parcel/research tooling, TBC basemap handoff docs | Active release blocker |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared drive not mounted in this session | Outside system-of-record folders cannot be verified from this host right now | Mount `G:\Shared drives\CERTASURV_PROJECT DRIVE` before final handoff validation |
| MACROTBC dirty release state | Production integration repo has many deleted/untracked/generated artifacts, so a blind push would be unsafe | Curate intended release files before staging and pushing |
| WV_COURTHOUSE_RESEARCHER missing origin | Release worker cannot push or inspect upstream state for the WV blocker repo from this host | Add `origin` with `Set-CertaGitRemotes.ps1 -Apply` after confirming repository access |
