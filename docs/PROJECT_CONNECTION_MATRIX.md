...
| GitHub CLI is installed but unauthenticated on this host | `gh`-driven release pushes and private workflow/log inspection are blocked until auth is restored, and the provisioning script now reports that explicitly | Run `gh auth login` with repo and workflow scopes |
| `node` and `npm` are installed on disk but missing from PATH in the current release shell | Node web tooling and any package-driven local checks remain blocked from this shell | Normalize host PATH or install/reinstall Node.js LTS when the machine is not under production load |
