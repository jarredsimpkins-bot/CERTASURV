...
| WV_COURTHOUSE_RESEARCHER has no visible `origin` on this host | Cloud offload, private workflow checks, and release review cannot cover the courthouse research lane yet | Create/confirm `wv-courthouse-researcher.git`, add `origin`, push `codex/wv-courthouse-researcher-cabell-lessons`, and resolve local changes |
