...
| CERTARD | `C:\Users\SimpS\OneDrive\Documents\CERTARD` | Shared-drive mount helpers; `origin` -> `https://github.com/jarredsimpkins-bot/certard.git` | Project watchlist, staging scripts | Active coordination repo; push/review timing lane |
| MACROTBC | `C:\Users\SimpS\OneDrive\Documents\MACROTBC` | AppSheet, Google Drive, command-center manifest; `origin` -> `https://github.com/jarredsimpkins-bot/macrotbc.git` | TBC macros, CAD resources, installers, sync scripts | Active production integration repo; substantive blocker until local package/config readiness is restored |
...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No remote configured during June 21 check | Courthouse/title research, parcel evidence, TBC-ready research package docs | Active local research repo; substantive blocker until remote/push policy is decided |
...
| Shared drive is not mounted in the June 21 provisioning shell | Release staging and handoff paths cannot be verified locally | Restore `G:` mount before shared-drive validation or handoff copy |
| MACROTBC package/config files are absent from the current working tree | MACROTBC local repo validation and shared-drive config checks cannot complete | Restore or intentionally regenerate `package.json` and `certasurv_shared_drive.json`, then rerun MACROTBC readiness checks |
| WV_COURTHOUSE_RESEARCHER has no configured remote | Courthouse research lane cannot be pushed or reviewed through GitHub from this repo state | Configure a GitHub remote or document a launch decision that it stays local-only |
