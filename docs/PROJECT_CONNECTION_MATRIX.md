...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Planned `origin` -> `https://github.com/jarredsimpkins-bot/wv-courthouse-researcher.git`; no remote configured locally during June 23 pass | Courthouse research toolkit, assessor prep docs, evidence packaging scripts | Active release blocker until upstream is configured and pushed |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared drive not mounted in this session | Outside system-of-record validation and generated handoff-copy steps are blocked until `G:` is restored | Re-run `scripts\Test-CertaProjectProvisioning.ps1 -Detailed` after Drive Desktop remounts `G:\Shared drives\CERTASURV_PROJECT DRIVE` |
...
| MACROTBC has active local release state | Production integration remains the substantive release blocker | Review and commit or discard the intended `command_center/tbc/command_usage_log.csv` change before launch handoff |
| WV_COURTHOUSE_RESEARCHER has no local `origin` remote | Release push/review cannot start from the active WV branch | Create or confirm access to `wv-courthouse-researcher`, then run `scripts\Set-CertaGitRemotes.ps1 -Apply` |
