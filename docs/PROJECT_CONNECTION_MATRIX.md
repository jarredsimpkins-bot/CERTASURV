...
| CERTARD | `C:\Users\SimpS\OneDrive\Documents\CERTARD` | Shared-drive mount helpers; `origin` -> `https://github.com/jarredsimpkins-bot/certard.git` | Project watchlist, staging scripts | Coordination repo; push/review timing only |
| MACROTBC | `C:\Users\SimpS\OneDrive\Documents\MACROTBC` | AppSheet, Google Drive, command-center manifest; `origin` -> `https://github.com/jarredsimpkins-bot/macrotbc.git` | TBC macros, CAD resources, installers, sync scripts | Release blocker until local command usage log change is triaged |
...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Expected GitHub repo `wv-courthouse-researcher` does not resolve through `gh`; no local `origin` configured | Courthouse/title research runbooks, toolkit scripts, and TBC prep docs | Release blocker until remote and local edits are triaged |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared drive mount unavailable on this host | Outside system-of-record folders cannot be verified until Google Drive Desktop remounts `G:\Shared drives\CERTASURV_PROJECT DRIVE` | Restore the mount before staged-release verification or generated-copy handoff |
...
| MACROTBC has local release drift | Command usage log is modified locally and may represent runtime evidence rather than release docs | Owner decides whether to commit, archive, or exclude it before launch review |
| WV_COURTHOUSE_RESEARCHER has no remote | Cloud offload and release review cannot run for this repo until the GitHub destination exists and `origin` is set | Create/confirm `jarredsimpkins-bot/wv-courthouse-researcher`, then set `origin` and push a reviewed branch |
| `npm` is not on PATH in this host shell | Node web tooling may work inconsistently even though `node` is available through the Codex runtime path | Normalize host PATH or install/reinstall Node.js LTS when the machine is not under production load |
