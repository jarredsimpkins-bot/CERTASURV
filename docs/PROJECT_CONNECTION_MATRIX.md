...
| CERTARD | `C:\Users\SimpS\OneDrive\Documents\CERTARD` | Shared-drive mount helpers; `origin` -> `https://github.com/jarredsimpkins-bot/certard.git` | Project watchlist, staging scripts | Active coordination repo; current launch focus is push/review timing |
| MACROTBC | `C:\Users\SimpS\OneDrive\Documents\MACROTBC` | AppSheet, Google Drive, command-center manifest; `origin` -> `https://github.com/jarredsimpkins-bot/macrotbc.git` | TBC macros, CAD resources, installers, sync scripts | Active production integration repo; blocked by local churn and missing tracked package/config files |
...
| `node` and `npm` are missing from PATH in this shell | Node web tooling may work inconsistently outside the explicit Node install path | Normalize host PATH or install/reinstall Node.js LTS when the machine is not under production load |
| MACROTBC tracked package/config files are missing locally | TBC integration release checks cannot be treated as review-ready | Restore or intentionally regenerate `package.json` and `certasurv_shared_drive.json`, then isolate generated outputs from committed release fixes |
| WV_COURTHOUSE_RESEARCHER has no remote/upstream | Courthouse toolkit/runbook fixes cannot be pushed or reviewed from this workspace | Configure the planned GitHub remote/upstream or explicitly defer courthouse researcher release review |
