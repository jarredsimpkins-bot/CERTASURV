...
| `node` and `npm` are currently absent on this host | Web-app packaging or Node-based validation cannot run from the release host right now | Install or restore Node.js LTS before launch steps that depend on web tooling |
