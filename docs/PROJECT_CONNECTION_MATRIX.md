...
| MACROTBC release evidence is not fully reviewable from this host | TBC command-center release cannot be closed without private check/log evidence or the missing local shared-drive config file | Authenticate `gh`, verify the branch/workflow state, and confirm command-center manifest/shared-drive config handoff evidence |
| WV_COURTHOUSE_RESEARCHER remote/upstream is not closed out | Cloud offload, private workflow checks, and release review cannot cover the courthouse research lane yet | Create/confirm `wv-courthouse-researcher.git`, add `origin`, push `codex/wv-courthouse-researcher-cabell-lessons`, and resolve local changes |
