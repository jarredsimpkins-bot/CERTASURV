...
Last updated: 2026-06-17 10:05 ET
...
| Outside operations drive | System-of-record folders, IDs, releases, and handoffs | `G:\Shared drives\CERTASURV_PROJECT DRIVE` is still the active command-center root; `G:\Shared drives\CERTASTRUCT` is mounted but not yet carrying `00_CERTASURV_COMMAND_CENTER` on this machine |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Drive-root naming is mid-cutover | `CERTASTRUCT` is mounted, but `current-projects-metadata.json` and the install runbook still point operators at `CERTASURV_PROJECT DRIVE` as the live root | Keep release/runbook language aligned to the live command-center root until the staged docs and filesystem root change together |
| CERTAHEALTH remote still points to `CERTASURV.git` | Source-control naming and release boundaries remain blurred | Cut over to `certahealth.git` only when the repo split is actually ready |
| GitHub CLI unauthenticated | Private workflow/artifact review is blocked from this environment | Run `gh auth login` with repo/workflow scopes before the next private-repo release pass |
