...
This file is the working standard and capability/connection map for making sure every active Certa/CertaSurv project has both outside connections and in-house production connections.
...
| Local Node toolchain is not on PATH on this host | Local Node-based release helpers may fail even though GitHub Actions can still run workflow checks | Restore Node.js / npm PATH or install Node.js LTS when the machine is not under production load |
