...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No remote listed in the current local checkout | Courthouse/title research package, ortho/LiDAR-to-TBC basemap docs, toolkit scripts | Release blocker until remote/upstream and dirty-tree state are resolved |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared drive mount missing in current shell | Outside system-of-record folders cannot be verified or staged from this host until `G:` is restored | Re-run the CERTARD drive mount helper, then rerun `scripts\Test-CertaProjectProvisioning.ps1 -Detailed` |
...
| GitHub CLI is authenticated with `repo` scope | Normal repository inspection and pushes can use `gh`; workflow dispatch/log operations may still need explicit `workflow` scope | Run `gh auth refresh -s workflow` only if workflow operations fail for scope reasons |
...
| MACROTBC working tree is dirty | Release packaging needs owner review before including or discarding command usage log changes | Review `command_center/tbc/command_usage_log.csv` in MACROTBC |
| WV_COURTHOUSE_RESEARCHER has no remote/upstream | Release review cannot be pushed from the current checkout as-is | Add or confirm the GitHub remote, then decide how to handle the local docs/scripts/tool-registry changes |
