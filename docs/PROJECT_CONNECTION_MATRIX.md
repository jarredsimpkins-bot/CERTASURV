...
| Courthouse research | WV courthouse/title research tooling and field package support | `WV_COURTHOUSE_RESEARCHER` docs, scripts, templates, and repo branch review |
...
| MACROTBC | `C:\Users\SimpS\OneDrive\Documents\MACROTBC` | AppSheet, Google Drive, command-center manifest; `origin` -> `https://github.com/jarredsimpkins-bot/macrotbc.git` | TBC macros, CAD resources, installers, sync scripts | Active production integration repo; launch blocker until dirty command usage log is resolved |
...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Courthouse/title research branch review and eventual GitHub push | Courthouse research scripts, runbooks, templates, and ortho/LiDAR workflow notes | Launch blocker until active branch changes are reviewed, verified, and pushed |
...
| MACROTBC active branch has dirty command usage evidence | Release cannot distinguish intentional command-center evidence from transient output | Review `command_center/tbc/command_usage_log.csv` and commit only if intentional |
| WV_COURTHOUSE_RESEARCHER active branch has dirty release-support files | Courthouse researcher launch lane is not review-ready | Verify and push the active branch after reviewing docs, scripts, and template registry changes |
