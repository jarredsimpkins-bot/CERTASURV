...
| CERTARD | `C:\Users\SimpS\OneDrive\Documents\CERTARD` | `origin` -> `https://github.com/jarredsimpkins-bot/certard.git`; shared-drive mount helpers | Project watchlist, staging scripts | Active coordination repo; push/review timing item |
...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No configured local remote observed; expected `jarredsimpkins-bot/wv-courthouse-researcher` did not resolve via `gh repo view` | Courthouse/title research package tooling | Active local blocker repo; needs remote/readiness decision |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared drive mount missing on this host | Outside system-of-record folders are not available locally for release handoff/copy steps | Restore Google Drive Desktop mount at `G:\Shared drives\CERTASURV_PROJECT DRIVE` before shared-drive operations |
...
| WV_COURTHOUSE_RESEARCHER remote is unresolved | WV courthouse release blocker work cannot be pushed/reviewed from the local repo until a remote exists | Decide or create the GitHub repository, then configure `origin` |
| MACROTBC remains a substantive release blocker | TBC integration readiness is outside this control repo and still needs its own review lane | Finish MACROTBC work in `C:\Users\SimpS\OneDrive\Documents\MACROTBC` before launch |
