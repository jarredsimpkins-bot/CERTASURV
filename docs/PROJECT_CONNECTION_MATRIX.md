...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No configured remote in inspected workspace | Courthouse research toolkit, TBC basemap workflow, CertaSurv prep/runbook docs | Active local release-toolkit edits; blocked from cloud review until remote/upstream is configured |
...
| `pwsh` is missing from PATH in this shell | Local execution does not match the GitHub Actions PowerShell Core environment | Install PowerShell 7 or run local checks with Windows PowerShell while keeping CI as the final PowerShell Core validation |
...
| WV_COURTHOUSE_RESEARCHER has no remote | Courthouse toolkit/runbook fixes cannot be pushed or reviewed from this workspace | Configure the planned GitHub remote/upstream or explicitly defer courthouse researcher release review |
