...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Courthouse research source/docs inventory; remote planned as `https://github.com/jarredsimpkins-bot/wv-courthouse-researcher.git` | Research runbooks, hub builder, and parcel/deed support sources | Local repo is coherent; remote still needs to be configured |
...
| `WV_COURTHOUSE_RESEARCHER` has no local `origin` yet | Repo-local release helpers can inventory it, but cloud offload and remote setup remain blocked until the remote is added | Run `Set-CertaGitRemotes.ps1 -Apply` there after the GitHub repo is confirmed |
