...
| CERTASURV_WEB_APP | `C:\Users\SimpS\OneDrive\Documents\CERTASURV_WEB_APP` | Shared-drive project data and estimate folders; `origin` -> `https://github.com/jarredsimpkins-bot/certasurv-web-app.git` | Local Python app/dashboard tools | Active local app project; control scripts now use the canonical path only |
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Needs remote/upstream decision before release publication | Courthouse research, ortho/LiDAR/TBC basemap package docs and tooling | Blocked by dirty worktree and missing remote listing in this workspace |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared drive mount unavailable in this run | Outside system-of-record folders could not be verified or staged from this host | Restore `G:\Shared drives\CERTASURV_PROJECT DRIVE` before shared-drive handoff |
...
| MACROTBC has a modified command usage log | Release state is not clean enough for push/review timing | Review, commit, or intentionally discard `command_center/tbc/command_usage_log.csv` in the MACROTBC repo |
| WV_COURTHOUSE_RESEARCHER lacks remote/upstream signal and has local edits | WV courthouse package remains a substantive release blocker | Set the intended remote/upstream and reconcile the modified docs/scripts/templates |
| GitHub CLI is authenticated | `gh`-driven release pushes and private workflow/log inspection are available when token scope is sufficient | Re-check `gh auth status` before release pushes |
