...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No `origin` configured yet | Courthouse/deed research runbooks, toolkit scripts, ortho/LiDAR-to-TBC prep docs | Active local blocker repo with uncommitted operational changes |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared drive not mounted in this session | Outside system-of-record folders cannot be re-verified locally | Restore Google Drive Desktop mount for `G:\Shared drives\CERTASURV_PROJECT DRIVE` before handoff |
...
| `node` and `npm` are missing from PATH in this session | Node web tooling cannot be run from the current shell | Normalize host PATH or install/reinstall Node.js LTS when the machine is not under production load |
| WV_COURTHOUSE_RESEARCHER has no remote and dirty local changes | Courthouse research package cannot be reviewed or pushed as part of launch until source-control destination is set | Review/commit current changes, then add an `origin` and push the active branch |
