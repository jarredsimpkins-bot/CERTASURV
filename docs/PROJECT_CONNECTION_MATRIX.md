...
| In-house local apps | Local web/dashboard/parcel/estimate tools | `CERTASURV_WEB_APP`, `WV_COURTHOUSE_RESEARCHER`, plus local Python runtime |
...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Courthouse/deed research outputs; no `origin` configured yet in the local repo | Local deed analysis, parcel evidence, and courthouse research tooling | Active local research workspace that now needs remote alignment |
