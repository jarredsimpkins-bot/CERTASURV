...
| CERTARD | `C:\Users\SimpS\OneDrive\Documents\CERTARD` | Shared-drive mount helpers; `origin` -> `https://github.com/jarredsimpkins-bot/certard.git` | Project watchlist, staging scripts | Active coordination repo; mainly push/review timing |
| MACROTBC | `C:\Users\SimpS\OneDrive\Documents\MACROTBC` | AppSheet, Google Drive, command-center manifest; `origin` -> `https://github.com/jarredsimpkins-bot/macrotbc.git` | TBC macros, CAD resources, installers, sync scripts | Active production integration repo; private cloud readiness runs are visible and failing; local branch is ahead by 2 |
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Planned `origin` -> `https://github.com/jarredsimpkins-bot/wv-courthouse-researcher.git`; courthouse/deed/GIS research package | Deed plotting, CAD resources, field-to-finish support, ortho/LiDAR basemap workflow | Active local repo; planned GitHub repo could not be resolved and local remote/upstream setup is blocked |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared drive mount unavailable in this session | Shared-drive handoff verification and package copy checks cannot run locally | Restore `G:\Shared drives\CERTASURV_PROJECT DRIVE` before final handoff validation |
...
| MACROTBC release evidence is failing | TBC command-center release cannot be closed while private cloud readiness runs fail and the local branch is ahead by 2 | Push the local branch state, fix the failing checks, and confirm command-center manifest/shared-drive config handoff evidence |
| WV_COURTHOUSE_RESEARCHER remote/upstream is not closed out | Cloud offload, private workflow checks, and release review cannot cover the courthouse research lane yet | Create or regain access to `wv-courthouse-researcher.git`, add `origin`, push `codex/wv-courthouse-researcher-cabell-lessons`, and resolve local changes |
| GitHub CLI is authenticated without explicit workflow scope | Private repo read/review is available, but workflow-scope operations may still need reauthentication | Refresh `gh` auth with workflow scope before workflow-file edits or dispatches |
