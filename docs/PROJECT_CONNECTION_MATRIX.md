...
| MACROTBC | `C:\Users\SimpS\OneDrive\Documents\MACROTBC` | AppSheet, Google Drive, command-center manifest; `origin` -> `https://github.com/jarredsimpkins-bot/macrotbc.git` | TBC macros, CAD resources, installers, sync scripts | Active production integration repo; release branch scope is blocked by dirty/generated/deleted local state |
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No configured Git remote observed on June 21, 2026 | Courthouse research toolkit docs/scripts and basemap workflow | Active research tooling repo; remote/upstream setup is a release blocker |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared drive not mounted in this session | Outside system-of-record folders cannot be inspected from this host until Drive Desktop exposes `G:` again | Remount `G:\Shared drives\CERTASURV_PROJECT DRIVE` before shared-drive handoff verification |
...
| MACROTBC release scope is unresolved | Dirty/deleted/generated local state makes a broad push unsafe | Split generated outputs from source/runbook changes before review |
| WV_COURTHOUSE_RESEARCHER has no remote | Modified toolkit files cannot be pushed or reviewed from this host until an upstream exists | Confirm target slug, then run `Set-CertaGitRemotes.ps1 -Apply` or add the remote manually |
