...
| --- | --- | --- | --- | --- |
| CERTAHEALTH | `C:\Users\SimpS\OneDrive\Documents\CERTAHEALTH` | `origin` currently points to `https://github.com/jarredsimpkins-bot/CERTASURV.git`; target publishable slug is `certahealth.git` when split | Laptop load manager, project provisioning scripts | Active local control repo |
...
| CERTASURV_WEB_APP | `C:\Users\SimpS\OneDrive\Documents\CERTASURV_WEB_APP` | `origin` -> `https://github.com/jarredsimpkins-bot/certasurv-web-app.git` | Local Python app/dashboard tools | Active local app project after workspace rename |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared drive mounted and staged | Outside system-of-record folders are available locally for handoff validation | Latest stage log remains `G:\Shared drives\CERTASURV_PROJECT DRIVE\00_CERTASURV_COMMAND_CENTER\08_REPORTS_EXPORTS\drive-stage-log-20260522-194113.txt` until a newer staging pass is recorded |
| Repo remote mapping is partially misaligned | `CERTAHEALTH` can push today, but it is still wired to the public `CERTASURV.git` repo instead of the planned `certahealth.git` slug | Decide whether to keep the shared control repo model or repoint `origin` with `scripts/Set-CertaGitRemotes.ps1 -Apply` |
| GitHub CLI installed but unauthenticated on this machine | Local `gh` workflow inspection and PR automation are blocked even though public repo pages remain reachable | Run `gh auth login` or provide `GH_TOKEN` before relying on authenticated release operations |
| Workspace rename needs final sweep outside this repo | This repo now refers to `CERTASURV_WEB_APP`, but some external/local tooling may still reference `New project2` until those repos are updated | Continue rename alignment in the web-app and coordination repos |
| `npm` availability should be rechecked from the primary workstation | This worktree can use bundled Node through Codex, but launch docs should not assume global Node on the host machine without confirmation | Verify `node` and `npm` on the production workstation during install/runbook validation |
