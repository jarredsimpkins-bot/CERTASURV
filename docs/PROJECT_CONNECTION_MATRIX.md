...
| CERTARD | `C:\Users\SimpS\OneDrive\Documents\CERTARD` | Shared-drive mount helpers; `origin` -> `https://github.com/jarredsimpkins-bot/certard.git` | Project watchlist, staging scripts | Push/review-timing lane; current sibling worktree has pre-existing local churn |
| MACROTBC | `C:\Users\SimpS\OneDrive\Documents\MACROTBC` | AppSheet, Google Drive, command-center manifest; `origin` -> `https://github.com/jarredsimpkins-bot/macrotbc.git` | TBC macros, CAD resources, installers, sync scripts | Substantive release blocker: production integration hardening and review remain open |
...
| CERTASURV_WEB_APP | `C:\Users\SimpS\OneDrive\Documents\CERTASURV_WEB_APP` | Shared-drive project data and estimate folders; `origin` -> `https://github.com/jarredsimpkins-bot/certasurv-web-app.git` | Local Python app/dashboard tools | Push/review-timing lane after workspace rename; current sibling worktree has pre-existing local churn |
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No `origin` remote observed during June 21 release-ops check | Deed, courthouse, ortho/LiDAR, and field-packet tooling | Substantive release blocker: local changes and missing upstream need resolution |
...
| Gap | Impact | Fix Path |
| --- | --- | --- |
| Shared drive not mounted on this host | Outside system-of-record folders cannot be inspected directly from this run | Restore Google Drive Desktop mount for `G:\Shared drives\CERTASURV_PROJECT DRIVE` before final handoff |
...
| WV_COURTHOUSE_RESEARCHER has no configured `origin` | The courthouse research lane cannot be pushed/reviewed by normal release automation | Create or choose the GitHub repository, then set `origin` and push the active branch |
...
| MACROTBC shared-drive config missing locally | TBC command-center package cannot prove shared-drive routing from this host | Restore or regenerate `C:\Users\SimpS\OneDrive\Documents\MACROTBC\certasurv_shared_drive.json` from the intended source of truth |
