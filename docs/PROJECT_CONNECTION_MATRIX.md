...
| CERTARD | `C:\Users\SimpS\OneDrive\Documents\CERTARD` | Shared-drive mount helpers; `origin` -> `https://github.com/jarredsimpkins-bot/certard.git` | Project watchlist, staging scripts | Active coordination repo; current launch focus is push/review timing once the intended clean release branch/worktree is selected |
...
| MACROTBC local release scope is unresolved | Generated outputs, command-center CSV deltas, and package/config deletions still make the production integration repo a launch blocker | Close or intentionally package the MACROTBC deltas in the MACROTBC repo before release handoff |
| WV_COURTHOUSE_RESEARCHER local release scope is unresolved | Toolkit/runbook/toolbox edits plus missing-directory warnings still make the courthouse researcher repo a launch blocker | Close the active WV_COURTHOUSE_RESEARCHER edits and repair missing-directory status warnings in that repo |
