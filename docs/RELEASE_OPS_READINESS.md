...
Last updated: 2026-06-23 01:25

## Launch Snapshot

| Area | Current Signal | Readiness |
| --- | --- | --- |
| Control repo workflow | GitHub CLI shows latest `main` run `#41` on June 18, 2026 succeeded; latest visible overall run `#181` on June 23, 2026 also succeeded | Ready |
...
| GitHub CLI access | `gh auth status` reports an authenticated `jarredsimpkins-bot` session with `repo` scope on this host | Ready |
...
- Latest verified overall run: `#181`, branch `codex/release-ops-blocker-refresh-20260623-0058`, commit `8da6dba`, created `2026-06-23T05:04:27Z`, conclusion `success`
- Verified job summary for run `#181`: single job `powershell-and-docs` succeeded, including `Validate PowerShell syntax` and `Verify required control files`
...
- Keep using GitHub CLI for private workflow visibility while the current credential remains valid; public REST remains a fallback for public control-repo checks.
