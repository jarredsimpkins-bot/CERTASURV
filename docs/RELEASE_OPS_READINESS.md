...
| --- | --- | --- |
| Control repo workflow | GitHub Actions shows latest overall run `#211` on June 24, 2026 succeeded on `codex/release-ops-gh-refresh-20260624`; latest `main` run `#41` also succeeded | Ready |
...
| WV_COURTHOUSE_RESEARCHER release lane | Local workspace exists but has no `origin`, no resolved `jarredsimpkins-bot/wv-courthouse-researcher` repo, and multiple uncommitted runbook/toolkit edits | Blocked |
...
- Latest verified overall public run: `#211`, branch `codex/release-ops-gh-refresh-20260624`, commit `c934499`, created `2026-06-24T04:22:37Z`, conclusion `success`
- Latest verified `main` run: `#41`, commit `6f40b64`, created `2026-06-18T16:17:45Z`, conclusion `success`
- Verified job summary for run `#211`: single job `powershell-and-docs` succeeded, including `Validate PowerShell syntax` and `Verify required control files`
