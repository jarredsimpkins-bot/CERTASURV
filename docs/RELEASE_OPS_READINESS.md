...
Last updated: 2026-06-24 02:18 UTC

## Launch Snapshot

| Area | Current Signal | Readiness |
| --- | --- | --- |
| Control repo workflow | GitHub CLI now shows latest visible run `#206` on June 24, 2026 succeeded on `codex/release-control-webapp-path-guard-20260624`; recent runs `#202`-`#206` are green | Ready |
...
| MACROTBC release gate | Latest `CertaSurv TBC Cloud Readiness` run `#37` failed on June 24, 2026; required checks failing are Command registry, TBC operator workflow HTML, and TBC tabulated workflow | Blocked |
...
- Latest verified run: `#206`, branch `codex/release-control-webapp-path-guard-20260624`, commit `7520332`, created `2026-06-24T01:57:13Z`, conclusion `success`
- Recent verified runs `#202` through `#206` all completed successfully on release-control/readiness branches.
...
| --- | --- | --- |
| CERTARD | `CERTARD Checks` run `#31` succeeded on `main` at commit `b2d1e59` on June 22, 2026 | Mostly push/review timing; not the primary release blocker |
| CERTASURV_WEB_APP | Latest visible CI remains run `#6`, failing June 19, 2026 on `tests/test_survey_storage.py::test_associated_drive_survey_export_dir_uses_active_project_lane` | Needs branch cleanup/push/review timing before treating web app as release-green |
| MACROTBC | `CertaSurv TBC Cloud Readiness` run `#37` failed June 24, 2026 at commit `daafac0`; required checks failing are Command registry, TBC operator workflow HTML, and TBC tabulated workflow | Primary release blocker |
...
2. Fix MACROTBC required readiness checks: Command registry, TBC operator workflow HTML, and TBC tabulated workflow.
