...
| --- | --- | --- |
| Control repo workflow | Public `CertaHealth Control Checks` latest visible run `#26` on June 17, 2026 succeeded in 19s | Ready |
| Control repo docs | Core launch docs are present in repo and now tracked by workflow required-file validation | Ready |
...
- Review the GitHub Actions Node.js 20 deprecation warning when updating workflow dependencies, even though the current public control check is still succeeding.
