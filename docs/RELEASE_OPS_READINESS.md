...
| --- | --- | --- |
| Control repo workflow | Public API shows latest overall visible run `#141` on June 22, 2026 UTC succeeded on branch `codex/release-control-webapp-path-guard-20260621-2301`; latest visible `main` run `#41` also succeeded | Ready |
...
| Web app workspace naming | Active local folder is `C:\Users\SimpS\OneDrive\Documents\CERTASURV_WEB_APP`; helper scripts now use that path directly and CI blocks legacy script references | Ready |
...
- Latest verified overall public run: `#141`, branch `codex/release-control-webapp-path-guard-20260621-2301`, commit `60ce5c0`, created `2026-06-22T00:03:59Z`, conclusion `success`
- Latest verified `main` run: `#41`, branch `main`, commit `6f40b64`, created `2026-06-18T16:17:45Z`, conclusion `success`
- Latest verified job summary for run `#141`: single job `powershell-and-docs` succeeded, including `Validate PowerShell syntax`, `Verify required control files`, and `Block legacy web app path in scripts`.
...
6. Restore Node/npm availability on PATH before web-app workflow/package checks that depend on local Node tooling.
