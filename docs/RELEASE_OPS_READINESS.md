...
| --- | --- | --- |
| Control repo workflow | Public API now shows latest `main` run `#41` and latest visible overall run `#78` succeeded on June 20, 2026 | Ready |
...
| MACROTBC release lane | `MACROTBC` has `origin` and an active upstream branch, but private workflow/log review still requires authenticated `gh` | Blocked for private review evidence |
...
During the June 20, 2026 local verification pass, the `G:` shared-drive mount and command-center folders were not available from this host, so external handoff verification is limited to repo-local path references until Drive Desktop is mounted again.

## Current Verified Workflow Signal

- Workflow file: `.github/workflows/certahealth-control-checks.yml`
- Latest verified `main` run: `#41`, branch `main`, commit `6f40b64`, created `2026-06-18T16:17:45Z`, conclusion `success`
- Latest verified visible overall run: `#78`, branch `codex/release-ops-wv-control-alignment-20260620`, commit `b0947d5`, created `2026-06-20T03:56:54Z`, conclusion `success`
- Verified public workflow signal: recent runs `#78`, `#77`, `#76`, `#75`, and `#74` all completed successfully

## Launch Blockers

1. Authenticate `gh` on this host with repo and workflow scopes so private workflow runs and logs can be inspected directly.
2. Confirm MACROTBC private workflow/review evidence after `gh` auth is restored; the repo has a remote/upstream, but private release signal is still not visible here.
3. Create or provide `https://github.com/jarredsimpkins-bot/wv-courthouse-researcher.git`, attach it as `origin`, and push `codex/wv-courthouse-researcher-cabell-lessons`.
4. Restore the `G:\Shared drives\CERTASURV_PROJECT DRIVE` mount before final handoff validation.
