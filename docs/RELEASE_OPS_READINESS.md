...
| Research workspace coverage | Release-control scripts now inventory `WV_COURTHOUSE_RESEARCHER` and surface its remote gap without treating the repo as missing | Ready with remote blocker |
...
3. Add the planned `origin` to the local `WV_COURTHOUSE_RESEARCHER` checkout when remote automation for that repo is needed.
