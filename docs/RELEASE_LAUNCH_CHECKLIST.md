...
| --- | --- | --- |
| Workflow green on `main` | Latest public `CertaHealth Control Checks` run for `main` is successful | Verified on June 21, 2026: run `#41`, commit `6f40b64`, success |
...
| Workspace path aligned | Operator-facing web app path uses `C:\Users\SimpS\OneDrive\Documents\CERTASURV_WEB_APP` | Required by scripts and guarded in CI against `New project2` fallback |
| WV research lane inventoried | Control scripts include `WV_COURTHOUSE_RESEARCHER` without assuming remote availability | Local-only remote state is expected until the GitHub target is attached |
...
3. `WV_COURTHOUSE_RESEARCHER` remains local-only in this host checkout until an `origin` remote is configured.
...
- Latest verified overall run during this pass: `#110` on branch `codex/release-ops-blocker-triage-20260621`, success on June 21, 2026; current `main` run `#41` also succeeded
