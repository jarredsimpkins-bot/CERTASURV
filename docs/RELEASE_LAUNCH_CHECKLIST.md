...
| Workspace path aligned | Operator-facing web app path uses `C:\Users\SimpS\OneDrive\Documents\CERTASURV_WEB_APP` | Legacy `New project2` fallback removed from control scripts |
| WV courthouse researcher lane | `WV_COURTHOUSE_RESEARCHER` is inventoried in provisioning, remote setup, publish, and cloud-offload helpers | Added to control scripts; local repo still needs remote/push review |
| Shared-drive handoff references intact | External staging/deploy references should point to `G:\Shared drives\CERTASURV_PROJECT DRIVE` | Blocked locally until the shared drive is mounted |
...
5. Review `WV_COURTHOUSE_RESEARCHER` local changes, set `origin` to `https://github.com/jarredsimpkins-bot/wv-courthouse-researcher.git`, then push the intended release branch.
...
2. `WV_COURTHOUSE_RESEARCHER` is present locally but needs release-change review plus remote/upstream configuration before it can join the launch push cadence.
3. `G:\Shared drives\CERTASURV_PROJECT DRIVE` is not mounted in this run, so live handoff staging cannot be revalidated here.
