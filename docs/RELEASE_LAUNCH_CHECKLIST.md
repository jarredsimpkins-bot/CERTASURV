...
| GitHub access for private repos | `gh` is authenticated with repo/workflow access on the host used for release operations | Ready on this host: `jarredsimpkins-bot` authenticated with `repo` scope |
...
6. Confirm `gh auth status` still shows the expected `jarredsimpkins-bot` account before any private workflow/log inspection or release push.
...
- Latest verified overall run during this pass: `#181` on branch `codex/release-ops-blocker-refresh-20260623-0058`, success on June 23, 2026
