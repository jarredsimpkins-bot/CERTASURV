...
| Local release shell available | `pwsh` is available for operator commands, or Windows PowerShell fallback is recorded | `pwsh` not on PATH during June 20, 2026 pass |
...
| Shared-drive handoff references intact | External staging/deploy references still point to `G:\Shared drives\CERTASURV_PROJECT DRIVE` | Repo-local references checked June 20, 2026; live mount absent in this session |
...
3. Local release execution is blocked by missing `pwsh`, Node/npm PATH, unmounted shared drive, and missing `MACROTBC\certasurv_shared_drive.json`.
...
- Latest verified overall run during this pass: `#103` on branch `codex/release-control-webapp-path-guard-20260620`, success on June 20, 2026; latest `main` run `#41` also succeeded
