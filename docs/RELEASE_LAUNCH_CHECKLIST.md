...
| GitHub access for private repos | `gh` is authenticated with repo/workflow access on the host used for release operations | Blocked: `gh auth status` reports no login |
| Local web tooling available | `node` and `npm` are available for any web-app or package build steps required during launch | Blocked on this host |
...
2. Run `powershell -File .\scripts\Test-CertaProjectProvisioning.ps1 -Detailed` and record any missing tools or mounts. On this host, `pwsh` is not on `PATH`.
...
7. Restore `node` and `npm` on the host before any release step that depends on web-app packaging or Node-based tooling.

## Current Launch Blockers

1. `gh` is not authenticated on this host, so private workflow runs/logs cannot be inspected here.
2. `node` and `npm` are missing on this host, so the provisioning baseline currently fails its tool check.
...
- Latest verified overall run during this pass: `#57` on branch `codex/wv-courthouse-inventory-hardening-20260618`, success on June 18, 2026; latest `main` run `#41` also succeeded
