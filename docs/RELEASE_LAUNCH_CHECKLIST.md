...
| CERTARD ready for release review | Repo-local launch edits are separated from generated/unrelated churn and branch timing is agreed | Review timing |
| CERTASURV_WEB_APP ready for release review | Web app branch timing is agreed and Node/npm checks can run from release shell | Review timing; Node/npm PATH blocked locally |
...
9. Keep CERTARD and CERTASURV_WEB_APP in push/review timing unless new evidence shows a release-blocking regression.
...
6. `node`, `npm`, and `pwsh` are not on PATH in this shell, blocking local release checks.
...
- Latest visible overall run during this pass: `#123` on branch `codex/release-control-webapp-path-guard-20260621-1320`, success on June 21, 2026
- Blocker exit criteria: `docs/RELEASE_BLOCKER_EXIT_CRITERIA_2026-06-21.md`
