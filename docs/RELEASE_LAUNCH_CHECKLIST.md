...
| --- | --- | --- |
| Workflow green on `main` | Latest public `CertaHealth Control Checks` run for `main` is successful | Verified: run `#41`, commit `6f40b64`, success on June 18, 2026 |
| Recent release-ops workflow green | Latest visible release-ops/control workflow runs are successful | Verified on June 20, 2026: runs `#75` through `#69` all succeeded |
...
| Shared-drive handoff references intact | External staging/deploy references still point to `G:\Shared drives\CERTASURV_PROJECT DRIVE` | Repo-local references verified June 20, 2026; live drive mount unavailable in this session |
...
2. Run `pwsh -File .\scripts\Test-CertaProjectProvisioning.ps1 -Detailed` when PowerShell 7 is on PATH; otherwise use `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\Test-CertaProjectProvisioning.ps1 -Detailed` and record any missing tools or mounts.
...
7. Keep `CERTARD` and `CERTASURV_WEB_APP` in the push/review timing lane unless new failures appear; focus release engineering on `MACROTBC` and `WV_COURTHOUSE_RESEARCHER`.
...
3. `MACROTBC` is clean locally on its tracked branch, but still needs release review/signoff before the TBC integration lane is launch-ready.
4. `WV_COURTHOUSE_RESEARCHER` has no configured remote/upstream from this host, so launch review cannot push, compare, or open a PR yet.
...
- Latest visible overall run during this pass: `#75`, branch `codex/release-ops-blocker-refresh-20260620`, success on June 20, 2026
