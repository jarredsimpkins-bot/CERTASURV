...
| Control repo destination decided | Final answer exists for `CERTASURV.git` vs `certahealth.git` | Blocked: `certahealth.git` currently returns `repository not found` |
| Stack blocker lanes narrowed | `CERTARD` and `CERTASURV_WEB_APP` are mainly timing/review lanes while `MACROTBC` and `WV_COURTHOUSE_RESEARCHER` remain substantive blockers | Verified from current local repo state |
...
3. Confirm the latest public control workflow run on `main` is green before handing off notes or switching remotes, and note the latest overall public run number for traceability.
4. Verify external handoff references remain read-only unless copying from committed repo docs.
5. Decide the permanent control-repo remote before final launch cutover so release notes, automation, and operator docs all point to one destination. If the dedicated repo path is chosen, create `certahealth.git` first.
6. Confirm `MACROTBC` and `WV_COURTHOUSE_RESEARCHER` have either been merged/reviewed or explicitly accepted as remaining blockers before calling the stack launch-ready.
...
2. The permanent GitHub destination for `CERTAHEALTH` is still unresolved between public `CERTASURV.git` and dedicated `certahealth.git`, and the dedicated repository URL is not live today.
3. `MACROTBC` and `WV_COURTHOUSE_RESEARCHER` remain the substantive dirty implementation lanes in the current launch stack.
...
- Latest verified overall run during this pass: `#69` on branch `codex/release-ops-wv-blocker-refresh-20260619`, success on June 19, 2026
