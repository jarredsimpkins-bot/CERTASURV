...
| Node tooling reachable | `node` and `npm` resolve on the host used for release tooling | Blocked on this host |
...
7. Normalize `node` and `npm` on `PATH` before any launch task that depends on Node-based tooling or package scripts.
...
3. `node` and `npm` are installed on disk but still missing from `PATH` in the checked PowerShell environment.
...
- Latest verified overall run during this pass: `#53` on branch `codex/wv-courthouse-launch-alignment-20260618c`, success on June 18, 2026
