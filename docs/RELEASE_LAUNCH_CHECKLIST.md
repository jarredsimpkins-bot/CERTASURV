...
| Release-ops host baseline | Provisioning script confirms repo paths and drive mounts; `node` and `npm` remain off `PATH` | Verified on June 18, 2026 |
...
5. If this host will also run web-app release tasks, add `node` and `npm` to `PATH` before that handoff.
