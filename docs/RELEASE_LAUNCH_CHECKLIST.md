...
| Workspace path aligned | Operator-facing web app path uses `C:\Users\SimpS\OneDrive\Documents\CERTASURV_WEB_APP` without script-side `New project2` fallback | Required by workflow |
| WV researcher inventory aligned | `WV_COURTHOUSE_RESEARCHER` is listed as local-only and remote-blocked until a destination is assigned | Required by workflow |
...
3. The shared drive is unmounted on this host for live handoff checks.
4. `WV_COURTHOUSE_RESEARCHER` remains local-only until a remote destination is assigned.
