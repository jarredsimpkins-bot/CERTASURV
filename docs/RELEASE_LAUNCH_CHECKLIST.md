...
| WV_COURTHOUSE_RESEARCHER remote ready | GitHub repo exists, `origin` is set, and local runbook/toolkit edits are committed or triaged | Blocked: no `origin`; expected repo does not resolve |
...
5. `npm` is not on PATH in the current host shell; use the explicit Node runtime path or normalize Node.js LTS before web-app release checks.
...
- Latest verified overall run during this pass: `#214` on branch `codex/release-control-webapp-path-guard-20260624-d012`, success on June 24, 2026
