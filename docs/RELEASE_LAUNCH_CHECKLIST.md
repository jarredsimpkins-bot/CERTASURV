...
| Local Node tooling callable | `node` and `npm` resolve in the release shell for any local web/package tasks | Blocked on this host |
...
3. Confirm the latest public control workflow run on `main` is green before handing off notes or switching remotes; current verified `main` evidence is run `#41` on commit `6f40b64`.
...
7. Restore `node` and `npm` to `PATH` before any local Node-based packaging or verification step.
...
3. `node` and `npm` are installed on disk but not callable from the current release shell.
...
- Latest verified overall run during this pass: `#55` on branch `codex/cloud-offload-detached-head-hardening-20260618`, success on June 18, 2026
