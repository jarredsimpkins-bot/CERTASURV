...
| WV researcher lane accounted for | `WV_COURTHOUSE_RESEARCHER` is visible in release-control inventory without assuming a remote | Local-only / remote-blocked |
...
7. Keep `WV_COURTHOUSE_RESEARCHER` local-only in automation until a remote is intentionally configured.
...
3. `WV_COURTHOUSE_RESEARCHER` remains remote-blocked by design.
