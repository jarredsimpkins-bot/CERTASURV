...
| MACROTBC repo readiness | Local branch has `origin` set and no uncommitted changes before release push/review | Observed clean on `codex/certasurv-unified-forward` on June 20, 2026 |
| WV courthouse repo readiness | Local branch has `origin`, upstream, and no uncommitted release-blocking changes | Blocked: no visible `origin`; local changes present on June 20, 2026 |
...
7. Run `.\scripts\Set-CertaGitRemotes.ps1` as a dry run and confirm `WV_COURTHOUSE_RESEARCHER` maps to `https://github.com/jarredsimpkins-bot/wv-courthouse-researcher.git` before applying.
8. Do not run cloud offload for WV courthouse work until its local changes are reviewed and an upstream branch exists.
...
3. `WV_COURTHOUSE_RESEARCHER` is not yet remote-ready from this host: no `origin` was visible and local changes are present.
