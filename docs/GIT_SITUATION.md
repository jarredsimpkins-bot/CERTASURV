...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No approved remote in the control matrix |
...
| WV_COURTHOUSE_RESEARCHER | Decision needed before publication |

All planned repositories except WV_COURTHOUSE_RESEARCHER now exist. `CERTASURV` is public; the remaining launch support repositories are private.
...
- GitHub CLI is authenticated on this host for `jarredsimpkins-bot` with `repo` scope as of June 23, 2026.
- CERTARD and `CERTASURV_WEB_APP` are release timing/review lanes; avoid staging their generated artifacts or local data into control-repo commits.
- MACROTBC and WV_COURTHOUSE_RESEARCHER remain the substantive release blockers: MACROTBC needs TBC command-center review, and WV_COURTHOUSE_RESEARCHER needs an approved remote/readiness path.
