...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No `origin` configured yet in the local repo |
...
- `WV_COURTHOUSE_RESEARCHER` is treated as locally coherent but still blocked on adding its remote in the local checkout.
