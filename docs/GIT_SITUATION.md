...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No local `origin` configured on this host |
...
Planned repository targets are documented. `CERTASURV` is public; the remaining launch support repositories are private and require authenticated access to verify from this host.
...
- `CERTARD` and `CERTASURV_WEB_APP` have known remotes and are mainly waiting on push/review timing.
- `MACROTBC` has a known remote but remains a substantive release blocker until dirty production artifacts are curated.
- `WV_COURTHOUSE_RESEARCHER` is a Git worktree but lacks local `origin`; configure it before release push/review.
- `CERTASURV_WEB_APP` is the active local workspace name; `New project2` is retired and should not be used by launch scripts.
- GitHub CLI is installed locally but unauthenticated, so private-repo workflow inspection still requires `gh auth login`.
