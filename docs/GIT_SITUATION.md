...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | local git only; no `origin` configured |
...
- `WV_COURTHOUSE_RESEARCHER` exists locally as a renamed active workspace and should be referenced by that exact path in launch docs and helper scripts.
