...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Local repo coherent; remote push blocked |
...
All planned repositories except `WV_COURTHOUSE_RESEARCHER` now exist or are already attached locally. `CERTASURV` is public; the remaining launch support repositories are private or planned private.
...
- `WV_COURTHOUSE_RESEARCHER` is locally coherent but has no local `origin`; remote creation/attachment is blocked until GitHub auth is available.
- GitHub CLI is installed locally but unauthenticated on June 20, 2026, so private-repo workflow inspection still requires `gh auth login`.
