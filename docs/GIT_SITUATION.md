...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No `origin` configured in the local repo |
...
- `WV_COURTHOUSE_RESEARCHER` is active locally and should be included in remote audits, cloud-offload checks, and release handoff docs.
- GitHub CLI is installed locally but remains unauthenticated on this host as of June 18, 2026, so private-repo workflow inspection still requires `gh auth login`.
