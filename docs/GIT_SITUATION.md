...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No `origin` configured in this run |
...
`CERTASURV` is public; the remaining launch support repositories are private. `gh auth status` succeeds on this host as `jarredsimpkins-bot` with `repo` scope as of June 22, 2026. The planned remote list now includes `WV_COURTHOUSE_RESEARCHER`.
...
- `CERTASURV_WEB_APP` is the active local workspace name; release scripts now use that path directly and the workflow blocks retired `New project2` script references.
- `CERTASURV_WEB_APP` currently has many generated/data/cache local changes in the observed checkout; review status before any push from that repo.
- `CERTARD` currently has one modified liaison brief in the observed checkout; review timing remains the main action there.
- `WV_COURTHOUSE_RESEARCHER` is the observed local-only release blocker: branch `codex/wv-courthouse-researcher-cabell-lessons` has uncommitted release-prep files and no configured remote.
- GitHub CLI is authenticated locally during this pass; re-run `gh auth status` before private workflow/log inspection.
