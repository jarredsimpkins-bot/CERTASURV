...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No remote configured in inspected workspace |
...
- `CERTASURV_WEB_APP` is the active local workspace name; control-repo scripts now use that path directly and the workflow rejects `New project2` references in scripts.
...
- CERTARD and CERTASURV_WEB_APP have remotes and are primarily waiting on push/review timing after local churn is triaged.
- MACROTBC and WV_COURTHOUSE_RESEARCHER remain the substantive release blockers: MACROTBC has a missing shared-drive config plus local churn, and WV_COURTHOUSE_RESEARCHER needs a remote/upstream.
