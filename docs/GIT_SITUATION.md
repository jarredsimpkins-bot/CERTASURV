...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No `origin` remote observed on June 20, 2026 |
...
- GitHub CLI is installed locally but remains unauthenticated during the June 20, 2026 release-ops pass, so private-repo workflow inspection still requires `gh auth login`.
- `WV_COURTHOUSE_RESEARCHER` is active on branch `codex/wv-courthouse-researcher-cabell-lessons` but has no observed remote, making WV handoff/push review a launch blocker until the remote is created and attached.
