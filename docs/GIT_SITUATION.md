...
| CERTARD | `C:\Users\SimpS\OneDrive\Documents\CERTARD` | `origin` -> `https://github.com/jarredsimpkins-bot/certard.git`; latest visible checks have a green `main` run and older branch failures |
| MACROTBC | `C:\Users\SimpS\OneDrive\Documents\MACROTBC` | `origin` -> `https://github.com/jarredsimpkins-bot/macrotbc.git`; latest visible readiness run is failing |
...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No `origin` configured; `gh repo view jarredsimpkins-bot/WV_COURTHOUSE_RESEARCHER` does not resolve |
...
| WV_COURTHOUSE_RESEARCHER | Decision needed |

All planned repositories except the WV courthouse researcher destination are known to exist. `CERTASURV` is public; the remaining launch support repositories are private.
...
- GitHub CLI is authenticated locally for `jarredsimpkins-bot` with `repo` scope, so private workflow inspection is available from this host.
- MACROTBC remains a release blocker until `CertaSurv TBC Cloud Readiness` passes.
- WV courthouse researcher remains a release blocker until a GitHub destination is chosen and configured as `origin`.
