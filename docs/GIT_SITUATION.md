...
- GitHub CLI is installed locally but `gh auth status` still reports no login during the June 18, 2026 release-ops pass, so private-repo workflow inspection still requires `gh auth login`.
