...
- GitHub CLI is installed locally but `gh auth status` still reports no logged-in host as of June 18, 2026, so private-repo workflow inspection still requires `gh auth login`.
