...
- `CERTASURV_WEB_APP` is clean locally and remains the active workspace name.
- `CERTARD` still has one modified file, `drive/CERTASURV_LIAISON_BRIEF.md`, so it is near-ready but not fully clean as of June 19, 2026.
- `MACROTBC` remains the largest dirty repo among the currently published remotes.
- `WV_COURTHOUSE_RESEARCHER` is still local-only with no `origin`, which keeps its release-doc cleanup and follow-up fixes off GitHub until the remote exists.
- GitHub CLI is installed locally but unauthenticated on this host, so private-repo workflow inspection still requires `gh auth login`.
