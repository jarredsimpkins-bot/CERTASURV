...
- `WV_COURTHOUSE_RESEARCHER` is now tracked as a launch repo; the local workspace is on `codex/wv-courthouse-researcher-cabell-lessons`, has uncommitted docs/toolkit changes, and has no configured remote.
- GitHub CLI is authenticated locally as `jarredsimpkins-bot`; private workflow lists are available, but workflow writes still require checking scopes before editing workflow files through GitHub APIs.
