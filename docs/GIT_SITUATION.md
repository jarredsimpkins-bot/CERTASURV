...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Local Git worktree without configured `origin`; remote-blocked locally |
...
`CERTASURV` is public; the remaining launch support repositories are private or planned private handoff targets. `WV_COURTHOUSE_RESEARCHER` is locally coherent, but this host still reports it as local-only because no `origin` is configured in that checkout.
...
- `CERTASURV_WEB_APP` is the active local workspace name; release-control scripts must not fall back to `New project2`.
