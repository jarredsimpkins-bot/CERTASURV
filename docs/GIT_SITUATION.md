...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No `origin` observed in this pass; branch has local uncommitted release-prep changes |
...
Planned repositories are expected under `jarredsimpkins-bot`. `CERTASURV` is public; private repository existence and workflow state cannot be confirmed in this run because `gh` is unauthenticated.
...
- `CERTASURV_WEB_APP` is the active local workspace name; control scripts now treat a missing `CERTASURV_WEB_APP` path as a provisioning gap instead of falling back to `New project2`.
- `WV_COURTHOUSE_RESEARCHER` is now part of the control repo's remote/provisioning inventory, but the local repo needs its current changes reviewed before remote setup and push.
- GitHub CLI is installed locally but unauthenticated in this run, so private-repo workflow inspection still requires `gh auth login`.
