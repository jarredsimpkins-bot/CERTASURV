...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | Local Git repo present; no `origin` configured on this host |
...
- `CERTASURV_WEB_APP` is the active local workspace name; release-control scripts should not fall back to the retired `New project2` path.
- `WV_COURTHOUSE_RESEARCHER` is locally coherent but remote-blocked until an `origin` is configured or created.
