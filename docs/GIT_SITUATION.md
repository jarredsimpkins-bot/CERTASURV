...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No `origin` configured |
...
- `CERTARD` and `CERTASURV_WEB_APP` have configured remotes and are in push/review timing rather than core blocker status.
- `WV_COURTHOUSE_RESEARCHER` needs a remote decision and review of its current uncommitted operational/runbook changes before it can be treated as launch-ready.
