...
All planned repositories now exist or have a reserved target name except for any WV repo that has not yet been created remotely. `CERTASURV` is public; the remaining launch support repositories are intended to remain private.
...
- `gh auth status` still reports no logged-in hosts on this machine, so private-repo workflow inspection still requires `gh auth login`.
- `CERTARD` and `CERTASURV_WEB_APP` are the closest lanes to release-ready, but the current local check still found one modified doc in `CERTARD`.
- `MACROTBC` and `WV_COURTHOUSE_RESEARCHER` are the substantive blockers: `MACROTBC` is dirty and `WV_COURTHOUSE_RESEARCHER` is both dirty and missing `origin`.
