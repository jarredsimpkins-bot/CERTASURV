...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No remote shown in local release-ops status output |
...
- `CERTASURV_WEB_APP` is the active local workspace name. Control scripts now avoid `New project2` fallback paths, and CI blocks reintroducing that fallback under `scripts/*.ps1`.
- GitHub CLI is authenticated locally as `jarredsimpkins-bot` with `repo` scope as of June 23, 2026.
- `CERTARD` and `CERTASURV_WEB_APP` are mainly push/review timing items; `MACROTBC` and `WV_COURTHOUSE_RESEARCHER` remain the substantive release blockers.
