...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No remote configured as of June 21, 2026 |
...
Known planned repositories exist for the core launch stack. `CERTASURV` is public; the remaining launch support repositories are private. WV_COURTHOUSE_RESEARCHER still needs its remote/upstream configured or verified before it can join the pushed release set.
...
- `CERTARD` and `CERTASURV_WEB_APP` are currently treated as push/review timing lanes for launch operations.
- `MACROTBC` and `WV_COURTHOUSE_RESEARCHER` are the substantive launch blockers: MACROTBC needs release-scope cleanup, and WV_COURTHOUSE_RESEARCHER needs remote/upstream setup.
