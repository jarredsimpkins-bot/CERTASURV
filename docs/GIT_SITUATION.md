...
- This checkout is currently detached at `b4ca310c3ccf12cf4e6807e8aee3e830be8583cc`, with `origin/main` at the same commit.
- The local rename to `CERTASURV_WEB_APP` is live on this machine; `C:\Users\SimpS\OneDrive\Documents\New project2` does not exist anymore.
- CERTAHEALTH still uses the public `CERTASURV.git` remote today; the planned `certahealth.git` cutover remains a future-state target rather than the current live remote.
- `gh auth status` currently reports no authenticated GitHub host, so private Actions and artifact review are blocked from this environment.
