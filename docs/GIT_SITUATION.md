...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No `origin` currently configured |
...
`CERTASURV` is public; `certard` and `macrotbc` are private and reachable through GitHub CLI. `certahealth` and `wv-courthouse-researcher` were not resolvable through `gh repo view` during the June 22 release-ops pass.
...
- GitHub CLI is authenticated locally for `jarredsimpkins-bot` with `repo` scope.
- `WV_COURTHOUSE_RESEARCHER` has local dirty changes and no configured `origin`; do not auto-publish it until the dirty work is reviewed and the intended private repo exists.
