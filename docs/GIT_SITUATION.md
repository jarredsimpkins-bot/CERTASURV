...
| WV_COURTHOUSE_RESEARCHER | `C:\Users\SimpS\OneDrive\Documents\WV_COURTHOUSE_RESEARCHER` | No `origin` configured; active branch `codex/wv-courthouse-researcher-cabell-lessons` |
...
Known planned repositories for `CERTARD`, `MACROTBC`, `AUTOMATIONS`, and `CERTASURV_WEB_APP` exist. `CERTASURV` is public; the launch support repositories are private. `WV_COURTHOUSE_RESEARCHER` still needs a GitHub remote to be created or provided.
...
- `WV_COURTHOUSE_RESEARCHER` is now included in control-repo provisioning, remote-alignment, publish, and cloud-offload inventories, but launch push remains blocked until the GitHub remote exists and is attached.
