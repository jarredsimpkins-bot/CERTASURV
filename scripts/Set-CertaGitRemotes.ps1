...
$repos = @(
    ...
    @{ Name = 'WV_COURTHOUSE_RESEARCHER'; Path = Join-Path $documents 'WV_COURTHOUSE_RESEARCHER'; Slug = 'certard' },
...
if (-not $Apply) {
    Write-Host ''
    Write-Host 'Dry run only. Re-run with -Apply to align local origins with the current GitHub repositories.'
}
