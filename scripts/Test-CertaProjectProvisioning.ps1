...
$projects = @(
    ...
    @{ Name = 'WV_COURTHOUSE_RESEARCHER'; Path = Join-Path $documents 'WV_COURTHOUSE_RESEARCHER'; Type = 'research-tooling' },
...
$projectRows = foreach ($project in $projects) {
    ...
    if ($exists) {
        ...
        if ($hasGit) {
            ...
            $hasRemote = ($LASTEXITCODE -eq 0 -and -not [string]::IsNullOrWhiteSpace($originUrl))
        }
    }
