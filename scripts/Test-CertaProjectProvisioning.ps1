...
$projects = @(
    ...
    @{ Name = 'WV_COURTHOUSE_RESEARCHER'; Path = Join-Path $documents 'WV_COURTHOUSE_RESEARCHER'; Type = 'local-only-research'; RemoteBlocked = $true },
...
$projectRows = foreach ($project in $projects) {
    ...
    if ($hasGit) {
        ...
        $hasRemote = [bool]$origin
    }

    [pscustomobject]@{
        Area = 'Project'
        Name = $project.Name
        Status = if (-not $exists) { 'MISSING' } elseif ($project.RemoteBlocked) { 'LOCAL_ONLY' } elseif ($hasGit -and -not $hasRemote) { 'LOCAL_ONLY' } else { 'OK' }
