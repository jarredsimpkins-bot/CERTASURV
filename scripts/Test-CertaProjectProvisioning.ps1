...
$connections = @(
    ...
    @{ Name = 'WV Courthouse Toolkit Runbook'; Path = Join-Path $documents 'WV_COURTHOUSE_RESEARCHER\docs\certasurv_toolkit_runbook.md'; Lane = 'courthouse-runbook' },
    @{ Name = 'WV Courthouse Tool Registry'; Path = Join-Path $documents 'WV_COURTHOUSE_RESEARCHER\templates\certasurv_tool_registry.json'; Lane = 'courthouse-tooling' },
