...
$rows = foreach ($repo in $repos) {
    ...
    if ($repo.LocalOnly) {
        [pscustomobject]@{ Repo = $repo.Name; Status = 'local-only-remote-blocked'; Branch = ''; Remote = ''; Detail = 'No approved remote target; skipped cloud offload' }
        continue
    }

    $branch = git -C $repo.Path branch --show-current
    if (-not $branch) {
        [pscustomobject]@{ Repo = $repo.Name; Status = 'detached-head'; Branch = ''; Remote = ''; Detail = 'Skipped push because checkout is detached HEAD' }
        continue
    }
