...
# Discover every writable non-C volume; USB SSDs and flash drives may report as Fixed.
$candidates=@()
try{
  $candidates=Get-Volume | Where-Object { $_.DriveLetter -and $_.DriveLetter -ne 'C' -and $_.FileSystem } | ForEach-Object {
    ...
    $ok=$false
    try { 'x' | Set-Content -LiteralPath $probe -Encoding ASCII -ErrorAction Stop; Remove-Item -LiteralPath $probe -Force -ErrorAction SilentlyContinue; $ok=$true } catch {}
...
Log "START node=$env:COMPUTERNAME drives=$($candidates.DeviceID -join ',')"

# Node policy: local runtime stays local; reusable automation is server-bound.
$role=[ordered]@{
  generated_at=(Get-Date).ToString('o');node=$env:COMPUTERNAME;role='WORKSTATION_NODE';
  keep_local=@('C:\Certa4010 when present','Active production working set','Installed CAD/GIS runtime','Drivers/licenses/hardware interfaces');
...
foreach($d in $candidates){$role|ConvertTo-Json -Depth 5|Set-Content (Join-Path $roots[$d.DeviceID] 'NODE_ROLE_POLICY.json') -Encoding UTF8}
...
$plan=@()
if(Test-Path 'C:\Certa4010'){$plan+=New-PlanItem 'C:\Certa4010' '02_SCRIPTS_TOOLS\Certa4010_RECOVERY_COPY' 'CertaNode runtime/config' $false}
foreach($x in @(
...
foreach($r in $scanRoots){
  $files+=Get-ChildItem -LiteralPath $r -File -Recurse -Force -ErrorAction SilentlyContinue | Where-Object { $_.Extension.ToLower() -in $exts -and ($_.FullName -match $keyword -or $_.Name -match $keyword) -and $_.Length -lt 50MB }
}
$files=@($files|Sort-Object FullName -Unique)
$i=0
foreach($f in $files){
  $i++
  $bucket=if($f.Extension -eq '.md'){'04_LIMITED_KNOWLEDGE_BASE'}else{'02_SCRIPTS_TOOLS'}
  $plan += [pscustomobject]@{Source=$f.FullName;Relative=(Join-Path $bucket ("{0:D5}_{1}" -f $i,$f.Name));Category=if($f.Extension-eq'.md'){'Curated knowledge'}else{'Selected script/workflow'};PurgeEligible=$true;Bytes=[int64]$f.Length;Files=1}
}

$repos=@()
foreach($r in $scanRoots){
  $repos+=Get-ChildItem -LiteralPath $r -Directory -Recurse -Depth 7 -Force -ErrorAction SilentlyContinue | Where-Object {$_.Name -eq '.git'} | ForEach-Object {$_.Parent.FullName}
}
...
if(-not $plan){throw 'No curated skills/scripts/repos/knowledge items were found on this workstation.'}

# Greedy bin-pack by bytes while reserving space. This intentionally uses every drive when possible.
$driveState=@{}
foreach($d in $candidates){$driveState[$d.DeviceID]=[int64]0}
$assignments=@()
$ordered=@($plan|Sort-Object Bytes -Descending)
$driveIndex=0
foreach($p in $ordered){
  # Seed the first N items one-per-drive so small drives are guaranteed participation when enough items exist.
  if($driveIndex -lt $candidates.Count){
    $d=$candidates[$driveIndex]
    $driveIndex++
  } else {
    $d=$candidates | Sort-Object @{Expression={$driveState[$_.DeviceID]};Ascending=$true} | Select-Object -First 1
  }
  # Avoid assigning an item to a drive that cannot reasonably hold it.
  if($p.Bytes -gt ($d.FreeSpace - $driveState[$d.DeviceID] - 100MB)){
    $fit=$candidates | Where-Object { $p.Bytes -le ($_.FreeSpace - $driveState[$_.DeviceID] - 100MB) } | Sort-Object @{Expression={$driveState[$_.DeviceID]};Ascending=$true} | Select-Object -First 1
    if($fit){$d=$fit}else{Log "SKIP_NO_SPACE source=$($p.Source) bytes=$($p.Bytes)";continue}
  }
...
# Run one worker per drive in parallel. Pass ONE payload object so PowerShell 5.1 cannot flatten item arrays.
$jobs=@()
foreach($d in $candidates){
  $items=@($assignments|Where-Object {$_.Drive -eq $d.DeviceID})
  if(-not $items){continue}
  $payload=[pscustomobject]@{Items=$items;DestRoot=$roots[$d.DeviceID];DriveId=$d.DeviceID}
  $jobs+=Start-Job -ArgumentList (,$payload) -ScriptBlock {
    param($payload)
    $items=@($payload.Items)
    $destRoot=[string]$payload.DestRoot
    $driveId=[string]$payload.DriveId
    $out=@()
    foreach($p in $items){
      $dest=Join-Path $destRoot $p.Relative
      try{
        $srcItem=Get-Item -LiteralPath $p.Source -ErrorAction Stop
        if(-not $srcItem.PSIsContainer){
          New-Item -ItemType Directory -Force -Path (Split-Path $dest -Parent)|Out-Null
          Copy-Item -LiteralPath $p.Source -Destination $dest -Force
          $h1=(Get-FileHash -LiteralPath $p.Source -Algorithm SHA256).Hash
          $h2=(Get-FileHash -LiteralPath $dest -Algorithm SHA256).Hash
          $ok=$h1 -eq $h2;$df=1;$db=(Get-Item $dest).Length;$rc='HASH'
        }else{
          New-Item -ItemType Directory -Force -Path $dest|Out-Null
          & robocopy.exe $p.Source $dest /E /COPY:DAT /DCOPY:DAT /R:1 /W:1 /XJ /FFT /MT:8 /NP /NFL /NDL | Out-Null
          $rc=$LASTEXITCODE
          $src=Get-ChildItem -LiteralPath $p.Source -File -Recurse -Force -ErrorAction SilentlyContinue
          $dst=Get-ChildItem -LiteralPath $dest -File -Recurse -Force -ErrorAction SilentlyContinue
          $sb=($src|Measure-Object Length -Sum).Sum;if($null-eq$sb){$sb=0}
          $db=($dst|Measure-Object Length -Sum).Sum;if($null-eq$db){$db=0}
          $df=@($dst).Count
          $ok=($rc-le7)-and(@($src).Count-eq$df)-and([int64]$sb-eq[int64]$db)
        }
        $safe=if($ok -and [bool]$p.PurgeEligible){'YES'}else{'NO'}
        $out+=[pscustomobject]@{DRIVE=$driveId;CATEGORY=$p.Category;SOURCE_PATH=$p.Source;DESTINATION_PATH=$dest;SOURCE_FILES=$p.Files;DEST_FILES=$df;SOURCE_BYTES=$p.Bytes;DEST_BYTES=[int64]$db;COPY_RESULT=$rc;VERIFIED=$ok;PURGE_ELIGIBLE=[bool]$p.PurgeEligible;DISPOSITION=if([bool]$p.PurgeEligible){'OFFLOAD_SERVER'}else{'KEEP_LOCAL_RECOVERY_COPY'};SAFE_TO_PURGE=$safe;COPIED_AT=(Get-Date).ToString('o');ERROR=''}
      }catch{
        $out+=[pscustomobject]@{DRIVE=$driveId;CATEGORY=$p.Category;SOURCE_PATH=$p.Source;DESTINATION_PATH=$dest;SOURCE_FILES=$p.Files;DEST_FILES=0;SOURCE_BYTES=$p.Bytes;DEST_BYTES=0;COPY_RESULT='ERROR';VERIFIED=$false;PURGE_ELIGIBLE=[bool]$p.PurgeEligible;DISPOSITION='COPY_ERROR';SAFE_TO_PURGE='NO';COPIED_AT=(Get-Date).ToString('o');ERROR=$_.Exception.Message}
      }
    }
    $out
  }
